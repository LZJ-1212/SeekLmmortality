...
| I04 | 接口鉴权 / 限流 | 90% | 口令 `PLAY_ACCESS_TOKEN`（可逗号多条）+ `X-Play-Token`。日限默认关闭；`ACTION_DAILY_LIMIT` 正整数才封顶。给朋友玩必须**配置口令**。本机直连即使配了口令也不验；穿透必验。 |
| I05 | 存档列表 | 80% | `GET /api/saves` + 存档页；公网按口令仓隔离，互看不见。单删/清空只动本仓。局内快照回滚见 `LoadModal`（S12）。账号仍缺 |
| I06 | 托管 | 规格 90% / 代码 55% | 前端 API 基址 `apiBase.ts`；L1 双隧道；**游玩 worktree（3000/5174）与更新服（3001/5175、`wendaocs_dev`）分开**。规格 [hosting.md](./hosting.md) · 步骤 [runbook.md](./runbook.md) 第 7 节。不要裸映射 3000 |
...
3. **每日行动次数上限**已实现，**默认不限次**（朋友试玩）；`.env` 里 `ACTION_DAILY_LIMIT` 设为正整数才启用。  
