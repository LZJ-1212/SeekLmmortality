...
- 本记录最后更新：2026-09-01（补：写齐技术文档：api/runbook/内容表/六套系统规格/架构）
...
| S21 | 意图识别与安全网关 | 分类行动、防注入、截断超长输入 | [intent_gateway.md](./intent_gateway.md) | 无（现直接喂 DeepSeek） | 规格 90% / 代码 0% | 无 | **L1 必做最小集（口令+日限+长度+黑名单）** |
...
| S25 | 多结局与图鉴成就 | 结局引擎、天道点、二周目天赋 | [endings.md](./endings.md) | 死亡锁有，无结局分支 | 规格 85% / 代码 10% | 死亡有测 | 规划中 |
...
| I04 | 接口鉴权 / 限流 | 0% | **L1 必做**：口令或邀请码 + 每存档每日行动上限 |
...
人天仍作量级：S20 约 8–15；S21 规则版 1–3、模型版再 3–8；S22 约 5–10；S23 约 8–14；S24 约 8–16；S25 约 5–10。

建议：**L1 只强制做 S21 最小集**；其余在朋友试玩之后再开。全文：

- [combat_build.md](./combat_build.md)
- [intent_gateway.md](./intent_gateway.md)
- [chronicle.md](./chronicle.md)
- [beasts.md](./beasts.md)
- [world_sim.md](./world_sim.md)
...
当前优先级（已按你的选择调整）：**L1 朋友试玩 > 功法/安检等玩法 > 人声 TTS**。BGM/SFX 可夹在玩法之间做，不挡 L1。

1 人天 ≈ 专注 6–8 小时。

| 阶段 | 内容 | 人天 | 金钱 | 是否挡 L1 |
|------|------|------|------|-----------|
| D0 | 规则级输入过滤 + 长度上限（S21 最小集） | 1–3 | 0 | **是** |
| D1 | 邀请口令或共享密码 + 每日行动上限 | 1–2 | 0 | **是** |
| D2 | 部署：小主机/内网穿透 + 进程不掉 + 简单 runbook | 1–3 | 0–80 元/月 | **是** |
| D3 | （可选）存档列表 UI，免手传 UUID | 2–4 | 0 | 否，体验加分 |
| D4 | （可选）CC0 BGM + 2～3 个 SFX | 2–5 | 0 | 否 |
| 之后 | S20 功法 → S22 记忆 → S25 结局 → S23/S24 | 见 2.3 | 0 | 否 |
| 更后 | 人声朗读 / RVC | 见声音规格 | 见第 5 节 | 否 |

L1 最低集：**D0+D1+D2 ≈ 3–8 人天**，不必等人声、不必改许可证。
...
2. **接口口令或邀请码**：`/api/action` 现在无鉴权，知道 URL 就能烧你的 DeepSeek。  
3. **每存档每日行动次数上限**（例如 30～80），防止挂机刷。  
4. **输入最长限制 + 拒绝明显注入句**（S21 最小集）。  
5. **进程保活**：朋友打开时后端/MySQL 必须在跑（你电脑开机，或云上一份）。  
6. **说明文档**：前端地址、若需创角、出问题找你。  
7. **CC0 BGM 可选**；人声不必。
...
| [combat_build.md](./combat_build.md) 等六份 | 规划系统规格 |
...
**L1 主线：** D0 输入过滤 → D1 口令与日限 → D2 部署给朋友。  
**可并行：** CC0 BGM + 少量 SFX（人声延后）。  
**L1 之后再开：** 功法构筑 → 安全网关加强 → 大事记 → 结局图鉴 → 灵兽/大势。
...
六套新系统正好补「构筑、安全、记忆、宠物、活世界、结局」——做完会 **像一款游戏**；现在已经 **像一个很好的内核**。
