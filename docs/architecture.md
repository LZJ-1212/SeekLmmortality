...
5. 后端再写库（气血、时间、背包等），战斗公式覆盖胜负，把结果与 `player` 行返回前端。岁月与死亡锁细则见 [player_state.md](./player_state.md)；加深落点见 [player_state_architecture.md](./player_state_architecture.md)。
6. 前端追加日志、渲染选项与状态卡。
