/** 修订：2026-09-03 23:40 +08 lzj — 闭关禁止乱写境界；以物换灵石须填 JSON
 * 修订：2026-09-05 14:51 +08 lzj — 交手击毙以本场气血为准
 * 修订：2026-09-05 15:27 +08 lzj — 战斗估伤字段不再参与结算
 */
...
export async function deduceAction(
  ...
  const lockedNumbersNote = hasLockedNumbers
    ? '其中涉及的气血/灵力/修为具体数值已由天道（后端）精确计算完毕并直接生效，你在 hp_delta/mp_delta/cultivation_delta 里只需填 0，无需也不能自己重复计算这部分数值，只管把这段指令转化为生动的剧情文字。闭关出关时境界以【天道最高指令】为准，禁止叙事写已晋入其它小境或大境。'
...
1. 只要这次行动触发了战斗/交手/厮杀，就必须在返回的 JSON 里填写 combat 字段，如实报告对手的境界与五行属性；对手境界要符合剧情逻辑（例如荒野遇到的杂鱼妖兽通常是炼气期，宗门长老可能是金丹甚至更高，不要乱设定）。续场时【天道最高指令】已锁定对手身份与敌势，禁止更换对手或改其大境。
2. 交手打出与挨打的数字由天道按玩家神识（攻）、道心（防）、遁速（速）与对手大境三维结算；combat 里即使填写 base_damage_to_player / base_damage_to_enemy 也不会被采信。你不需要也不能自己套用"减伤60%"，也不能用 9999 点破局。
3. 剧情描写要体现境界威压：高出一个大境界，绝不能被低境界玩家轻易反杀；高出两个大境界，直接碾压秒杀，任何气运、神兵、法宝宣称都无法改写。玩家境界更高时，则应体现居高临下、几乎不受伤害的压倒性优势。
4. 对手是否气绝、交手是否结束，以【天道最高指令】与天道结算为准。禁止只凭文笔写死对手或写交手已歇；气血未绝则必须仍在交手。玩家明确逃走时可以写脱身。
5. 若这次行动根本不涉及战斗（如赶路、聊天、买东西），且天道未写交手未歇，combat 填 null 或省略即可。
【因果铁律·功德业力法则】：
...
【修仙百艺铁律】：玩家闭关时获得的修为增长，已由天道依据洞府灵气浓度（见下方"洞府"信息）、灵根、资质、道心精确计算并锁定，你的 cultivation_delta 此时必须填 0。闭关调息时气血/灵力恢复亦由天道结算，hp_delta 与 mp_delta 同样填 0。出关后境界不会只因闭关而变，禁止叙事写「已晋入后期/圆满/筑基」等，除非【天道最高指令】明确写了突破成功。玩家炼丹/炼器/阵法/灵植时，成败与产出也已由天道依据悟性/神识硬性判定并锁定在【天道最高指令】里：这种情况下 item_changes 里绝对不能出现那件被炼制物品本身的条目（无论成功还是失败，产出与否完全由天道处理，你重复声明就会导致数量翻倍或产出与矛盾），你只负责把结果写成生动剧情。洞府灵气越浓郁，闭关收益越高，可以在叙事里体现"灵气盈室、事半功倍"或"灵气稀薄、收效甚微"的氛围。
【宗门势力铁律】：
...
3. 私下以物换灵石时：spirit_stones_delta 须给合理正值，item_changes 里须用负 change 扣减背包中**名称完全一致**的物品；叙事写了成交但 JSON 不填，面板不会变。
【术法真实性铁律】：
